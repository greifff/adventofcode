...
//					System.out.println("$ " + p1.x + " " + p1.y);
			...
			photons = photons2.stream().filter(p1 -> 0 <= p1.y && p1.y < input.size())
					.filter(p1 -> 0 <= p1.x && p1.x < input.get(0).length()).toList();
		}
	...
	private static void part2(List<String> input) {
		...
		for (int x = 0; x <= maxx; x++) {
			touched = Math.max(touched, Math.max(processConfiguration(input, new Photon(x, 0, 2)),
					processConfiguration(input, new Photon(x, maxy, 0))));
		}
		for (int i = 0; i <= maxy; i++) {
			touched = Math.max(touched, Math.max(processConfiguration(input, new Photon(0, i, 1)),
					processConfiguration(input, new Photon(maxx, i, 3))));
		}
