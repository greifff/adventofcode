...
public class Day20 {
	...
	static MapNode walkRegex(String regex) {
		...
		for (int i = 0; i < regex.length(); i++) {
			...
			case 'E':
			case 'W':
				currentNode = currentNode.step(c);
				break;
			}
	...
	static void findLongestDirectPath(MapNode node) {
		Set<MapNode> visited = new HashSet<>();
		Set<MapNode> visiting;
		Set<MapNode> toVisit = new HashSet<>();
		int steps = -1;
		int farAway = 0;
		toVisit.add(node);
		...
		while (!toVisit.isEmpty()) {
			...
			if (steps >= 999) {
				farAway += visiting.size();
			}

			for (MapNode node1 : visiting) {
				toVisit.addAll(Arrays.asList(node1.north, node1.west, node1.south, node1.east));
			}
		...
		System.out.println("Longest path: " + steps);
		System.out.println("Rooms with path lengths>=1000: " + farAway);
	}

	static void printCount(MapNode node) {
		System.out.println("Node count: " + node.nodeRegistry.nodes.values().stream().flatMap(m -> m.values().stream()).count());
	}
}
