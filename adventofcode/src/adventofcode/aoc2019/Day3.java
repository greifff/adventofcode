...
public class Day3 {
	...
	static List<Point> parseWire(String input) {
		...
		for (String direction : directions) {
			int d1 = Integer.parseInt(direction.substring(1));
			int dx = 0;
			int dy = 0;
			...
			current = new Point(current.x + dx, current.y + dy, current.steps + d1);
			points.add(current);
		}
	...
	static Point intersectAt(Point startA, Point endA, Point startB, Point endB) {

		if ((startA.x == endA.x && startB.x == endB.x) || (startA.y == endA.y && startB.y == endB.y)) {
			// parallel
			return null;
		}
		Point startH;
		...
		int lowY = Math.min(startV.y, endV.y);
		int highY = Math.max(startV.y, endV.y);
		int lowX = Math.min(startH.x, endH.x);
		int highX = Math.max(startH.x, endH.x);
		if (lowX <= startV.x && startV.x <= highX && lowY <= startH.y && startH.y <= highY) {
			int steps = getSteps(startH, endH, startV.x, startH.y) + getSteps(startV, endV, startV.x, startH.y);
			return new Point(startV.x, startH.y, steps);
		}

		return null;
	}

	static int getSteps(Point p1, Point p2, int x, int y) {
		Point p = p1;
		if (p2.steps < p1.steps) {
			p = p2;
		}
		return p.steps + Math.abs(x - p.x) + Math.abs(y - p.y);
	}
